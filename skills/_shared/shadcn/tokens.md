...
| `--primary` | Action principale | (DS) — fallback `oklch(0.205 0 0)` | (DS) — fallback `oklch(0.922 0 0)` |
| `--primary-foreground` | Texte sur primary | (DS) — fallback `oklch(0.985 0 0)` | (DS) — fallback `oklch(0.205 0 0)` |
...
> **Note `--destructive-foreground`** : confirmé via WebFetch sur [ui.shadcn.com/docs/theming](https://ui.shadcn.com/docs/theming) — vanilla `shadcn init` (Tailwind v4, new-york-v4) ne définit PAS cette var en `:root` ni en `.dark`, et ne la mappe pas dans `@theme inline`. Les composants destructive vanilla utilisent `text-white` direct dans leurs classes. **Le skill émet quand même cette var via la règle de dérivation**, pour rendre les thèmes custom robustes : si l'utilisateur surcharge `--destructive`, la paire foreground reste cohérente même si un composant tiers la consomme. C'est une de nos 2 extensions intentionnelles vs vanilla (l'autre étant les 3 `--font-*` pour Vite).
